...
{
    public class GreenFeeController : BaseResellerCRUDController
    {
        #region Constructor
        ...
        public ActionResult Settings(int? TopNavIconGreenFeeCatID1 = null, int? TopNavIconGreenFeeCatID2 = null)
        {
            ...
            string[] AllowTeeSheetCategoryValues = Request.Form.GetValues("AllowTeeSheetCategories");
            string AllowTeeSheetCategoriesStr = AllowTeeSheetCategoryValues != null ? String.Join(",", AllowTeeSheetCategoryValues) : "";
            DataAccess.SaveOption("AllowTeeSheetCategories", AllowTeeSheetCategoriesStr);
            List<int> AllowTeeSheetCategories = new List<int>();
            AllowTeeSheetCategoryValues.ToList().ForEach(it =>
            {
                AllowTeeSheetCategories.Add(DataManager.ToInt(it.Trim()));
            });
            ViewBag.AllowTeeSheetCategories = AllowTeeSheetCategories;
        ...
        {
            List<Item> models = DataAccess.GetAllItems(param, ItemType.Type.GreenFee, Auth.User.SiteId, this.CultureId);
            return models;
        }
        ...
        //private void SaveTeeSheets(int id)
        //{
        //    DataAccess.DeleteTeeSheetsByItemId(id);
        ...
        //        foreach (TeeSheetModel teeSheet in teeSheetList.teeSheets)
        //        {
        //            teeSheets.Add(new TeeSheet()
        //            {
        //                ItemId = id,
        //                CourseId = DataManager.ToInt(Request.Form["CourseId"]),
        //                TeeSheetDate = DataManager.ToDateTime(teeSheet.date),
        //                TeeSheetDay = DataManager.ToShort(teeSheet.day),
        //                FromTime = new TimeSpan(DataManager.ToInt(teeSheet.start), 0, 0),
        //                ToTime = new TimeSpan(DataManager.ToInt(teeSheet.end), 0, 0),
        //                Price = DataManager.ToDecimal(teeSheet.price),
        //                Discount = DataManager.ToDecimal(teeSheet.discount)
        //            });
        //        }
        //        DataAccess.AddTeeSheets(teeSheets);
        //    }
        //    //string[] teeSheetDateArray = Request.Form["teesheet_dates[]"].Replace("-", "").Split(',');
        //    //string[] teeSheetStartTimeArray = Request.Form["teesheet_start_times[]"].Split(',');
        //    //string[] teeSheetEndTimeArray = Request.Form["teesheet_end_times[]"].Split(',');
        //    //string[] teeSheetPriceArray = Request.Form["teesheet_prices[]"].Split(',');
        //    //string[] teeSheetDiscountArray = Request.Form["teesheet_discounts[]"].Split(',');
        //    //for (int i = 0, length = teeSheetPriceArray.Length; i < length; i++)
        //    //{
        //    //    teeSheets.Add(new TeeSheet()
        //    //    {
        //    //        CourseId = DataManager.ToInt(Request.Form["CourseId"]),
        //    //        TeeSheetDate = DataManager.ToDateTime(teeSheetDateArray[i]),
        //    //        FromTime = new TimeSpan(DataManager.ToInt(teeSheetStartTimeArray[i]), 0, 0),
        //    //        ToTime = new TimeSpan(DataManager.ToInt(teeSheetEndTimeArray[i]), 0, 0),
        //    //        Price = DataManager.ToDecimal(teeSheetPriceArray[i]),
        //    //        Discount = DataManager.ToDecimal(teeSheetDiscountArray[i])
        //    //    });
        //    //}
        //}
        private void SaveTeeSheets(int id)
        {
            ...
            DateTime teeSheetDateFrom = DateTime.Today;
            DateTime teeSheetDateTo = DateTime.Today;
            DateTime.TryParseExact(Request.Form["TeeSheetDateFrom"], "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out teeSheetDateFrom);
            DateTime.TryParseExact(Request.Form["TeeSheetDateTo"], "dd/MM/yyyy", CultureInfo.InvariantCulture, DateTimeStyles.None, out teeSheetDateTo);
            DataAccess.DeleteTeeSheetsByItemId(id, teeSheetDateFrom, teeSheetDateTo);
            List<TeeSheet> teeSheets = new List<TeeSheet>();
            TeeSheetList teeSheetList = new JavaScriptSerializer().Deserialize<TeeSheetList>(Request.Form["TeeSheetData"]);
            List<TeeSheetModel> teeSheetModels = teeSheetList.teeSheets;
            ...
            if (teeSheetList.teeSheets != null && teeSheetList.teeSheets.Any())
            {
                teeSheetList.teeSheets = teeSheetList.teeSheets.Where(it => it.price > 0).OrderBy(it => it.date).ToList();

                for (DateTime day = teeSheetDateFrom.Date; day <= teeSheetDateTo; day = day.AddDays(1))
                {
                    var models = teeSheetModels.Where(it => it.day == ((int)day.DayOfWeek) + 1).ToList();
