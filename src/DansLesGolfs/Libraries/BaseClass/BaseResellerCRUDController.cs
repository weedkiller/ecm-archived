...
{
    public class BaseResellerCRUDController : BaseResellerController
    {
        #region Fields
        ...
        #region Constructors
        public BaseResellerCRUDController()
            : base()
        {
        }
        #endregion
        ...
        protected void Init()
        {
            ...
            ViewBag.Title = !String.IsNullOrEmpty(TitleName) ? TitleName : "Reseller Dashboard";
        ...
        protected object[] ConvertIEnumerableToArray(IEnumerable<object> result)
        {
            if (result != null)
            {
                ...
                foreach (var it in result)
                {
                    ...
                    {
                        row.Add("<a href=\"" + Url.Content("~/" + AreaName + "/DlgCard/" + "EditDlgCardReseller/?itemId=" + pkid) + "\" class=\"edit-link\">Edit</a>");
                        row.Add("<a href=\"javascript:void(0);\" data-id=\"" + pkid + "\" class=\"delete-link\">Delete</a>");
                    }
                    else if (ObjectName == "Promotion")
                    {
                        row.Add("<a href=\"" + Url.Content("~/" + AreaName + "/PromotionReseller/" + "EditPromotion/?promotionId=" + pkid) + "\" class=\"edit-link\">Edit</a>");
                        row.Add("<a href=\"javascript:void(0);\" data-id=\"" + pkid + "\" class=\"delete-link\">Delete</a>");
                    }
                    else
                    {
        ...
        public ActionResult Index()
        {
            ...
            {
                return RedirectToAction("PromotionList", "PromotionReseller");
            }
            else
            {
            ...
            try
            {
                ...
                if (ExceptionHelper.Count == 0)
                {
                    if (DoSave())
                    {
                        ...
                        TempData["SuccessMessage"] = TitleName + " has been saved.";
