...
jQuery(document).ready(function ($) {
    ...
    $('input[name=ItemName]').blur(function (e) {
        ...
        $('input[textarea=InvoiceName]').each(function () {
            if ($(this).val().trim() == '')
                $(this).val(val);
        });
    });
    ...
    $('#product-image-file').uploadifive({
        ...
        fileSizeLimit: '4MB',
        uploadScript: getUrl('Reseller/Item/AddItemImages'),
    ...
    $('#SiteId').change(checkAlbatrosSiteId);
    ...
    jQuery('#CategoryId').change(onItemCategoryIsChanged);
});
...
function onAddPriceButtonIsClicked(e) {
    ...
    var html = '<tr data-id="@price.ItemPriceId">' +
                ...
                '<td class="column-price"><input type="text" class="text-right" min="0" name="SpecialPrices" value="0" /><input type="hidden" name="PriceTypes" value="' + priceType + '" /></td>' +
...
function generateItemSlug(text) {
    jQuery.ajax({
        url: getUrl('Reseller/Item/GenerateItemSlug'),
...
function generateItemSlugByItemName() {
    var $el = jQuery('input[name=ItemName]').filter(function () { return this.value.trim().length !== 0; });
    if ($el.length > 0) {
        generateItemSlug($el.eq(0).val());
    }
}
...
function onItemCategoryIsChanged(e) {
    if (!window.itemId || window.itemId <= 0)
        generateItemCode();
}

function generateItemCode() {
    jQuery.ajax({
        url: getUrl('Common/AjaxGenerateItemCode'),
        data: { itemTypeId: window.itemTypeId, itemCategoryId: jQuery('#CategoryId').val() },
        dataType: 'json',
        type: 'post',
        success: function (result) {
            if (result.isSuccess) {
                jQuery('#ItemCode').val(result.itemCode);
            } else {
                console.error(result.message);
            }
        },
...
function saveAllItemImages() {
    ...
    $.ajax({
        url: getUrl('Reseller/Item/SaveItemImages'),
        type: 'post',
        dataType: 'json',
        traditional: true,
...
function loadItemImages() {
    $.ajax({
        url: getUrl('Reseller/Item/LoadItemImages'),
        type: 'post',
        dataType: 'json',
        traditional: true,
...
function onItemImageDelete(e) {
    if (confirm("Your image will be deleted permanently. Are you sure for deleting it?")) {
        ...
        $.ajax({
            url: getUrl('Reseller/Item/DeleteItemImage'),
            type: 'post',
            dataType: 'json',
...
function loadItemModifiers() {
    ...
    $.ajax({
        url: getUrl('Reseller/Item/LoadItemModifiers'),
        type: 'post',
        dataType: 'json',
        traditional: true,
        data: { id: window.itemId },
...
function addModifier(modifier) {
    $.post(getUrl('Reseller/Item/AddItemModifier'), { id: window.itemId, modifierId: modifier.modifierId }, function (result) {
        if (result.isSuccess) {
        ...
        $choice.tagsinput({
            typeahead: {
                source: function (query) {
                    return $.getJSON(getUrl('Reseller/Item/GetChoicesJSON'), { modifierId: $modifier.data('id') });
                }
            }
        });
    }
...
function saveModifiers(modifierIds, isVariants, isActives) {
    $.ajax({
        url: getUrl('Reseller/Item/SaveItemModifiers'),
...
function saveModifiersChoices($modifier) {
    ...
    $choices.each(function (i) {
        ...
        if (choices.length > 0) {
            jQuery.ajax({
                url: getUrl('Reseller/Item/SaveModifierChoices'),
...
function reorderModifiers() {
    ...
    jQuery.ajax({
        url: getUrl('Reseller/Item/ReorderItemModifier'),
...
function loadVariations() {
    jQuery.ajax({
        url: getUrl('Reseller/Item/AjaxLoadVariations'),
        data: { id: window.itemId },
        dataType: 'json',
        type: 'post',
        success: function (result) {
...
/*********************** Misc ****************************/
function checkAlbatrosSiteId(e) {
    var siteId = jQuery('#SiteId').val();
    var found = false;
    if (window.albatrosSiteIds) {
        for (var i in albatrosSiteIds) {
            if (albatrosSiteIds[i] == siteId) {
                found = true;
                break;
            }
        }
    ...
    if (found) {
        jQuery('a[href=#greenfee-teesheet]').parent().hide();
    } else {
        jQuery('a[href=#greenfee-teesheet]').parent().show();
    }
}