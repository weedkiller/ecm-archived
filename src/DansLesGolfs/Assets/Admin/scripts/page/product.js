...
jQuery(document).ready(function ($) {
    ...
    $('.test-tracking-code-button').click(onTestTrackingCodeClicked);
    ...
    $('input[name=ItemName]').blur(function (e) {
        ...
        $('textarea[name=InvoiceName]').each(function () {
            if ($(this).val().trim() == '')
                $(this).val(val);
        });
    });
    ...
    jQuery('#CategoryId').change(onItemCategoryIsChanged).trigger('change');

    // Force check albatros course id.
...
/*********************** Tracking Code ****************************/
function onTestTrackingCodeClicked(e) {
    e.stopPropagation();

    var formGroup = jQuery(this).closest('.form-group');
    var status = formGroup.find('.test-status');
    var codeTextBox = formGroup.find('textarea'); console.log(codeTextBox);

    status.html('');

    if (codeTextBox.length <= 0)
        return;

    jQuery.ajax({
        url: getUrl('ItemAjax/AjaxTestTrackingCode'),
        data: { code: codeTextBox.val() },
        type: 'post',
        dataType: 'json',
        success: function (result) {
            if (result.isSuccess) {
                jQuery.post(getUrl('ItemAjax/AjaxShowTempConversionTrackingCode'));
                status.html('<label class="label label-success">' + result.message + '</label>');
            } else {
                status.html('<label class="label label-danger">' + result.message + '</label>');
            }
        },
        error: function (xhr, msg) {
            status.html('<label class="label label-danger">' + msg + '</label>');
        }
    });
}