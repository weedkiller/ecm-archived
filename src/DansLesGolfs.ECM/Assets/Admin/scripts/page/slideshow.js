...
function addSlideImages(images) {
    ...
    $imagesList.html('');
    for (var i in images) {
        $imagesList.append(getSlideImageHTML(images[i].ImageName, images[i].ImageUrl, images[i].Description));
        addLastSlideImageEvent();
    }
}

function getSlideImageHTML(imageName, imageUrl, description) {
    var html = '<li data-image-name="' + imageName + '" class="row slide-image">';
