...
DROP PROCEDURE [dbo].[SelectECMUnsubscribersList]
GO

/****** Object:  StoredProcedure [dbo].[SelectECMUnsubscribersList]    Script Date: 28/2/2559 16:36:27 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [dbo].[SelectECMUnsubscribersList]
	@Start INT = NULL,
	@Length INT = NULL,
...
BEGIN
	...
	SET @SQL = @SQL + ' WHERE UserTypeId = 2 AND Active = 1 AND IsSubscriber = 0 AND IsReceiveEmailInfo = 0 '	
	...
	SET @SQL = @SQL + CHAR(13) + 'SELECT COUNT(*) AS TotalRows FROM Users WHERE UserTypeId = 2 AND Active = 1 AND IsSubscriber = 0 AND IsReceiveEmailInfo = 0'
